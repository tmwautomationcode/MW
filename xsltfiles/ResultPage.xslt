<xsl:stylesheet version="1.0" 
				...
				<body>
					...
					<div id="ReportRightcolumn" class="rightcontent">
						<div id="{//TestCase[@id=$testCaseID]/@id}">
							<h1 align="center" style="color:brown;"><xsl:value-of select="//TestCase[@id=$testCaseID]/@id" />: <xsl:value-of select="//TestCase[@id=$testCaseID]/@description" /></h1>
							<hr/>
							<div align="left">
								<table width="80%" >
					...
					<div id='buttonsar'>
						<a class='btn' href='../SummaryPage.html'>Click here to see the Summary page</a> <a class='btn' href='../../Report.html'>Click here to see the dashboard report</a>
					</div>
				</body>
			</html>
	</xsl:template>	
	...
	<xsl:template match="TestStep">
		...
		<tr>
			...
			<td style='width: 80%'>
				<font class="ReportFail"><xsl:value-of select="@Message" /></font>
			</td>
			</xsl:if>
			<xsl:if test="@Status='Pass'">
			<td style='width: 80%'>
				<font class="ReportPass"><xsl:value-of select="@Message" /></font>
			</td>
			</xsl:if>
			<xsl:if test="@Status='Info'">
			<td style='width: 80%'>
				<font class="ReportInfo"><xsl:value-of select="@Message" /></font>
			</td>
			</xsl:if>
			<xsl:if test="@Status='Warn'">
			<td style='width: 80%'>
				<font class="ReportWarn"><xsl:value-of select="@Message" /></font>
			</td>
			</xsl:if>
		</tr>
		<xsl:if test="@Status='Fail'">
